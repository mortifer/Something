...
class BarChart extends React.Component {
    render() {
        const { organizationId, data, renderX = x => x } = this.props;
        const maxValue = data.map(item => item.y).reduce((x, y) => Math.max(x, y), 0);
        return (
                        ...
                        {({h}) => {
                            ...
                            return (
                                <div className="cols_col" style={{height: h + '%'}}>
                                    <Link to={"/" + organizationId + "/Statistics/?from=" + shortDate + "&to=" + shortDate}>{renderX(item.x)}</Link>
                                </div>
                            )
                        }}
                    </Motion>
